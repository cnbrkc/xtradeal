...
class DonanimHaberScraper:
    ...
    # ── SON SAYFADAN GERİYE TARAMA ──
    def scrape_latest(self, num_pages: int = 5,
                      start_page: int = 0) -> tuple:
        """
        Son sayfadan geriye tarar.
        Returns: (posts, last_scanned_page, total_pages)
        """
        all_posts: List[ForumPost] = []
        total_pages = 0

        try:
            # 1) Toplam sayfa sayısını öğren
            ...
            # 3) Son sayfadan geriye tara
            end_page = max(1, start_page - num_pages + 1)
            print(f"[SCRAPER] Sayfa {start_page} → {end_page} taranacak "
                  f"({start_page - end_page + 1} sayfa, tersten)")

            last_scanned = start_page
            for pg in range(start_page, end_page - 1, -1):
                url = self._get_page_url(pg)
                print(f"  Sayfa {pg}: {url}")
        ...
        print(f"[SCRAPER] Toplam {len(all_posts)} post "
              f"(sayfa {start_page}→{last_scanned}).")
        return all_posts, last_scanned, total_pages
